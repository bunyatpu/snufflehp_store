...
const getProductPromote = (cb) =>{
  ...
  const newEntryRef = firebase.database().ref().child('newEntry');
...
const saveProductDB = (newProduct) => {
  ...
  let fnDb =  firebase.database().ref('/products').orderByChild('no').limitToLast(1).once('value').then( (snap) =>{
    //console.log('snap',snap.val());
    ...
    return firebase.database().ref('/products').push(newProduct);
...
const delProductDB = (key) => {
  ...
  return firebase.database().ref('/products').child(key).remove()
}
